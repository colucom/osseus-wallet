...
const getAbi = (config, contractName) => config.data.get(`WALLET_ABI`) ?
  config.data.get(`WALLET_ABI`)[contractName] : require(path.join(cwd, config.data.get('WALLET_ABI_PATH'), contractName))

const createContract = (contractName) => {
  const addresses = require(path.join(cwd, config.data.get('WALLET_CONTRACT_PATH')))
  const web3 = get()
